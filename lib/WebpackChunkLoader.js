 ...
 */
function loadSingleChunk(chunkName, path) {
    ...
    if (chunkRequests.has(chunkName))
        return chunkRequests.get(chunkName);
    ...
    const request = new Promise((resolve, reject) => {
        ...
        if (!chunkDefinitions.has(chunkInternal))
            return reject("Invalid chunk \"" + chunkName + "\" requested!");
...
export class WebpackChunkLoader {
    ...
    static registerLoader(definitions, options) {
        ...
        if (typeof options.timeout === "number")
            chunkLoaderOptions.timeout = options.timeout;
        if (typeof options.additionalEvents === "string")
            chunkLoaderOptions.additionalEvents = options.additionalEvents;
        if (typeof options.domChangeOnChunkLoad === "boolean")
            chunkLoaderOptions.domChangeOnChunkLoad = options.domChangeOnChunkLoad;
        // Get current url
        let currentUrl = "";
        ...
        $globj.document.on(events, function webpackChunkLoaderListener(e, requiredChunk, callback, onError) {
            ...
            WebpackChunkLoader
                ...
                if (typeof callback === "function")
                    callback(requiredChunk);
            })
                .catch(err => {
                if (typeof onError === "function")
                    onError(requiredChunk, err);
                else
                    throw new Error("Could not load required chunk: \"" + requiredChunk + "\"");
            });
        });
    }
    /**
     ...
     */
    static loadChunks(requiredChunks) {
        return new Promise((resolve, reject) => {
            // Convert input into a set if it is an array
            if (Array.isArray(requiredChunks)) {
                const requiredChunksTranslated = new Set();
                forEach(requiredChunks, v => {
     ...
     * @param $scope
     */
    static loadChunksInScope($scope) {
