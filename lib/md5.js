...
export function md5(value) {
    ...
    for (; i < str; i += 16) {
        for (a = h, j = 0; j < 64;)
            a = [d = a[3], (b = 0 | a[1]) + ((d = a[0] + [b & (c = a[2]) | ~b & d, d & b | ~d & c, b ^ c ^ d, c ^ (b | ~d)][a = j >> 4] + (k[j] + (0 | x[[j, 5 * j + 1, 3 * j + 5, 7 * j][a] % 16 + i]))) << (a = [7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21][4 * a + j++ % 4]) | d >>> 32 - a), b, c];
        for (j = 4; j;)
            h[--j] = h[j] + a[j];
    }
    str = "";
