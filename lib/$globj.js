...
class $globj {
    static get document() {
        if (!storage.has('document'))
            storage.set('document', $(document.documentElement));
        return storage.get('document');
    }
    static get window() {
        if (!storage.has('window'))
            storage.set('window', $(window));
        return storage.get('window');
    }
    static get html() {
        if (!storage.has('html'))
            storage.set('html', $('html'));
    ...
    static get htmlBody() {
        if (!storage.has('htmlBody'))
            storage.set('htmlBody', $('html, body'));
