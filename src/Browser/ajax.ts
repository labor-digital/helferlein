...
{
    if (!isBrowser()) {
        throw new Error('ajax() only works in browsers!');
    }
    if (!isPlainObject(request)) {
        throw new Error('Invalid request object given!');
    }
    ...
    return new Promise((resolve, reject) => {
        if (!isUndefined(request.data)) {
            request.data = prepareData(request.data, isGet);
        } else {
            request.data = null;
        }
