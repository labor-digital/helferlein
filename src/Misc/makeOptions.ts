...
export class OptionApplier {
	...
	protected applyInternal(list: List | any, definition: MakeOptionsDefinition, path: Array<string>): List | any {
		...
		if (listType === ListType.NoList) throw new Error("The given \"list\" is not a proper list object!");
		...
		forEach(definition, (def: MakeOptionsValueDefinition, k: string) => {
			...
			if (!isUndefined(getListValue(list, k))) return;
		...
		forEach(result, (v, k: string) => {
			...
			if (isUndefined(definition[k])) {
				...
				if (this.options.allowUnknown === true) return;
				// Handle the error
				this.errors.push("Invalid option key: \"" + k + "\" allowed keys are: " + Object.keys(definition).join(", "));
				return;
			}
			...
			if (!this.checkTypeValidation(v, def, path)) return;
			
			// Apply the filter
			v = this.applyFilter(k, v, def, path, list);
			
			// Check custom validation
			if (!this.checkCustomValidation(k, v, def, path, result)) return;
			
			// Check value validation
			if (!this.checkValueValidation(v, def, path)) return;
	...
	protected validateDefinition(def: MakeOptionsValueDefinition, path: Array<string>): void {
		...
		forEach(def, (v, k) => {
			if (ALLOWED_DEFINITION_KEYS.indexOf(k) === -1)
				throw new Error("Error at: \"" + path.join(".") + "\" invalid key found: \"" + k + "\" allowed keys are: " + ALLOWED_DEFINITION_KEYS.join(", "));
		});
	}
	...
	protected applyDefaultsFor(list: List, k: string, def: MakeOptionsValueDefinition, path: Array<string>): void {
		...
		if (isUndefined(def.default)) {
			if (!isUndefined(def.type) && (isString(def.type) && def.type === "undefined" || isArray(def.type) && def.type.indexOf("undefined") !== -1))
				return;
	...
	protected applyPreFilter(k: string, v: any, def: MakeOptionsValueDefinition, path: Array<string>, list: List): any {
		if (!isFunction(def.preFilter)) return v;
	...
	protected checkTypeValidation(v: any, def: MakeOptionsValueDefinition, path: Array<string>): boolean {
		...
		if (isUndefined(def.type)) return true;
		
		// Resolve shorthand
		let types = def.type;
		if (isString(types)) types = [types as MakeOptionTypes];
		...
		forEach(types as Array<MakeOptionTypes>, (type: string) => {
			...
			else throw new Error("Error at \"" + path.join(".") + "\" - Invalid type: \"" + type + "\" given! Only " + Object.keys(LIST_TYPE_MAP).join(", ") + " are allowed!");
		});
		
		// Validate the value
		if (!this.validateTypesOf(v, typeList)) {
			this.errors.push("Invalid value type (" + typeof v + ") at: \"" + path.join(".") + "\"; Allowed types are: " + (types as Array<string>).join(", "));
			return false;
		}
		return true;
	}
	...
	protected applyFilter(k: string, v: any, def: MakeOptionsValueDefinition, path: Array<string>, list: List): any {
		if (!isFunction(def.filter)) return v;
	 ...
	 * @param list The whole list for the callback
	 */
	protected checkCustomValidation(k: string, v: any, def: MakeOptionsValueDefinition, path: Array<string>, list: List): boolean {
		// Skip, if there is no validation required
		if (!isFunction(def.validator)) return true;
	...
	protected checkValueValidation(v: any, def: MakeOptionsValueDefinition, path: Array<string>): boolean {
		...
		if (!isArray(values)) {
			if (!isArray(this.customValidatorValues)) return true;
		...
		// Fail
		this.errors.push("Validation failed at: \"" + path.join(".") + "\"; The given value is not in the list of possible values!");
		return false;
	}
