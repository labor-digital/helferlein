 ...
 */
export function parsePath(path, separator?: string): Array<string> {
	if (isUndefined(path) || path === null || isArray(path) && (path as Array<string>).length === 0 || path === "") return [];
	if (!isString(path) && !isNumber(path) && !isArray(path))
		throw new Error("The given path: " + JSON.stringify(path) + " is not valid! Only strings, numbers and arrays are supported!");
	
	// Check if the given path array is valid
	if (isArray(path)) {
		// Path is array
		const validPathParts: Array<string> = filter(path, (v) => isString(v) || isNumber(v) || isArray(v));
		if (validPathParts.length !== path.length)
			throw new Error("The given path array: " + JSON.stringify(path) + " should only contain numbers, strings and arrays!");
		...
		if (!isUndefined(cachedPaths[cacheKey])) return cachedPaths[cacheKey].slice(0);
		...
		if (pathString.indexOf("[") !== -1) {
			...
			for (let i = 0; i < pathString.length; i++) {
				...
				if (char === "[") braces.set(braceId, braces.get(braceId) + "\\(\\b" + ++braceId + "\\)");
				if (!braces.has(braceId)) braces.set(braceId, "");
				braces.set(braceId, braces.get(braceId) + char);
				if (char === "]") braceId = Math.max(0, braceId - 1);
			}
			...
			braces.forEach((v, k) => {
				...
				// Split at the comma
				braces.set(k, JSON.stringify(
					map(
						v.substr(1, v.length - 2).match(/(\\.|[^,])+/g),
						(v: string) => {
							v = v.trim();
							// Resolve child paths
							if (v.indexOf(separator) !== -1) return parsePath(v, separator);
							return v;
						})));
		...
		let parts = map(
			pathString.match(new RegExp("(\\\\.|[^" + escapeRegex(separator) + "])+", "g")),
			(v: string) => v.trim()
		);
		...
		if (hasEscapedSeparators) {
			const pattern = new RegExp("\\\\" + escapeRegex(separator), "g");
			parts = map(parts, (v: string) => {
