...
function getTypeOfElement(element: any): 'array' | 'set' | 'map' | 'object' | null {
...
function getNewOutputElement(outputType: 'array' | 'set' | 'map' | 'object') {
	switch (outputType) {
		case 'array':
			return [];
		case 'set':
...
function genericGet(type: 'array' | 'set' | 'map' | 'object', list, key) {
	switch (type) {
		case 'array':
			return list[key];
		case 'set':
...
function genericSet(outputType: 'array' | 'set' | 'map' | 'object', output, key, value) {
	switch (outputType) {
		case 'array':
			// Don't duplicate array entries
			if (output.indexOf(value) !== -1) return output;
			output.push(value);
			return output;
		case 'set':
			output.add(value);
