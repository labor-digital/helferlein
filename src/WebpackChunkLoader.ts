...
const callbackQueue: Map<string, Array<WebpackChunkLoaderCallbackInterface>> = new Map();
...
export class WebpackChunkLoader {
	...
	static registerLoader(definitions: WebpackChunkLoaderDefinitionInterface[],
		...
		$globj.document.on(events, function webpackChunkLoaderListener(
			e: JQuery.Event, chunk?: string, callback?: WebpackChunkLoaderCallbackInterface
		) {
			...
			if (typeof callback === "function") {
				if (!callbackQueue.has(chunk)) callbackQueue.set(chunk, []);
			...
			(new Promise((resolve, reject) => {
				...
				if (loadedChunks.has(chunk)) return resolve(true);
				// Check if chunk is waiting
				if (waitingChunks.has(chunk)) return resolve("wait");
				// Check if we have this chunk
				if (!chunkDefinitions.has(chunkInternal)) return reject("Invalid chunk \"" + chunk + "\" requested!");
				...
				timeoutMarker = setTimeout(() => reject("Could not load the required chunk: " + chunk),
	...
	static registerDomChangeChunkLoading() {
		...
		$globj.document.on("domChange", (e) => {
			...
			forEach(requiredChunks, (chunk) => {
				...
				if (loadedChunks.has(chunkInternal)) return;
				// Fail silently if we don't know the chunk
				if (!chunkDefinitions.has(chunkInternal)) return;
				// Require the chunk
